...
if __name__ == "__main__":
    ...
    # Initial state
    x0_claptrap = np.zeros((12, 1))
    x0_claptrap[2,0] = np.random.rand(1) # Random yaw angle, this should have no impact on simulation
    x0_claptrap[4,0] = np.random.rand(1) # Random initial pitch angle
    
    # Initialize analytical simulation
    x0 = np.array([x0_claptrap[4,0], 0.0, 0.0, 0.0, 0.0])
